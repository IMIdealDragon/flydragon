...
void CSocekt::flyd_event_accept(lp_connection_t oldc)
{
    ...
    lp_connection_t newc;              //代表连接池中的一个连接【注意这是指针】
    ...
    do   //用do，跳到while后边去方便
    {
        ...
        if(s == -1)
        {
            ...
            LOG_ERROR << "CSocekt::ngx_event_accept()中accept4()失败!";
        ...
        if(newc == NULL)
        {
            ...
            if(close(s) == -1)
            {
                ...
                LOG_ERROR << "CSocekt::ngx_event_accept()中close(%d)失败!";
            }
            return;
        }
