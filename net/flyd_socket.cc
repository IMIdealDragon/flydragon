...
{
    ReadConf();  //读配置项
    bool reco = flyd_open_listening_sockets();  //打开监听端口
    return reco;
}
...
// 非阻塞，概念在五章四节讲解的非常清楚【不断调用，不断调用这种：拷贝数据的时候是阻塞的】
...
int CSocekt::flyd_epoll_init()
{
    ...
    if (m_epollhandle == -1)
    {
      ...
      LOG_FATAL << "CSocekt::ngx_epoll_init()中epoll_create()失败.";
    ...
    //连接池【数组，每个元素是一个对象】
    m_pconnections = new flyd_connection_t[m_connection_n]; //new不可以失败，不用判断结果，如果失败直接报异常更好一些
    ...
    lp_connection_t next  = NULL;
    lp_connection_t c = m_pconnections; //连接池数组首地址
    do
    {
    ...
    //lpngx_listening_t是指向一个监听结构体指针
    std::vector<lp_listening_t>::iterator pos;
    for(pos = m_ListenSocketList.begin(); pos != m_ListenSocketList.end(); ++pos)
    {
        //绑定一个监听套接字fd和连接对象
        c = flyd_get_connection((*pos)->fd); //从连接池中获取一个空闲连接对象，传入一个监听socket文件描述符
        if (c == NULL)
        {
            //这是致命问题，刚开始怎么可能连接池就为空呢？
            //ngx_log_stderr(errno,"CSocekt::ngx_epoll_init()中ngx_get_connection()失败.");
            LOG_FATAL << "CSocekt::ngx_epoll_init()中ngx_get_connection()失败.";
            exit(2); //这是致命问题了，直接退，资源由系统释放吧，这里不刻意释放了，比较麻烦
        }
        ...
        //对监听端口的读事件设置处理方法，因为监听端口是用来等对方连接的发送三路握手的，
        //所以监听端口关心的就是读事件
        c->rhandler = &CSocekt::flyd_event_accept;

        //往监听socket上增加监听事件，从而开始让监听端口履行其职责
        //【如果不加这行，虽然端口能连上，但不会触发ngx_epoll_process_events()里边的epoll_wait()往下走】
        if(flyd_epoll_add_event((*pos)->fd,       //socekt句柄
...
//c：对应的连接池中的连接的指针
//返回值：成功返回1，失败返回-1；
int CSocekt::flyd_epoll_add_event(int fd,
